...
(defun get-base-10 (list-digits base &optional (total 0) (remaining-length (length list-digits)))
  "Returns the base 10 representation of LIST-DIGITS in BASE."
  (let* ((current-digit (first list-digits))
         (current-exponent (1- remaining-length)))
    (cond ((= remaining-length 0) total)
          (t (get-base-10 (rest list-digits) base (+ total (* current-digit (expt base current-exponent))))))))
